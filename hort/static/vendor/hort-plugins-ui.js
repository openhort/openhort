/**
 * hort-plugins-ui.js — Plugin integration into the main openhort UI.
 *
 * Responsibilities:
 * 1. Discovers plugins via /api/plugins and loads their UI scripts
 * 2. Generates thumbnail previews for plugin cards (every 5s)
 * 3. Manages active plugin panels (auto-show, sort by last access)
 * 4. Provides <hort-plugin-manager> config panel
 *
...
(function (root) {
  ...
  async function discoverAndLoadPlugins() {
    try {
      const resp = await fetch(bp + '/api/plugins');
      if (!resp.ok) return;
    ...
    app.component('hort-plugin-manager', {
      setup() {
        ...
        async function toggleFeature(pluginId, feature, enabled) {
          await fetch(bp + '/api/plugins/' + pluginId + '/features/' + feature, {
