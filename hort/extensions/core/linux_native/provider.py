"""Native Linux window management via X11 tools.

Runs X11 commands directly on the local system (no Docker).
Requires: wmctrl, xdotool, ImageMagick (import), xdpyinfo.

X11 tools used:
- ``wmctrl -l -p -x -G`` — list windows with PID and WM_CLASS
...
class LinuxNativeExtension(ExtensionBase, PlatformProvider):
    """Native Linux PlatformProvider — runs X11 commands directly."""

    def __init__(self) -> None:
        self._display = os.environ.get("DISPLAY", ":99")

    def activate(self, config: dict[str, Any]) -> None:
        self._display = config.get("display", self._display)

    # -- helpers --

    def _env(self) -> dict[str, str]:
        """Environment with DISPLAY set."""
        env = os.environ.copy()
    ...
    def _exec_sync(self, cmd: str, timeout: float = 5) -> tuple[int, str, str]:
        ...
        result = subprocess.run(
            ["sh", "-c", cmd],
            capture_output=True, timeout=timeout, env=self._env(),
        )
        return (
            result.returncode,
    ...
    def list_windows(self, app_filter: str | None = None) -> list[WindowInfo]:
        ...
        # Virtual Desktop entry (full screen capture) — same as macOS
        if not app_filter:
    ...
    def capture_window(
        ...
        if window_id == DESKTOP_WINDOW_ID:
            # Capture full X11 root window (entire desktop composited)
    ...
    def handle_input(
        ...
        if event.type == "click":
            ...
            if is_printable and not non_shift_mods:
                self._exec_sync(
                    f"xdotool type --clearmodifiers -- {shlex.quote(key)}", timeout=t
                )
            else:
