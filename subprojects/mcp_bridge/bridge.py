"""MCP Bridge Server — serves in-process MCPMixin tools over MCP protocol.
...
class MCPBridge:
    ...
    async def handle_message(self, msg: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if method == "initialize":
            ...
            client_version = msg.get("params", {}).get("protocolVersion", PROTOCOL_VERSION)
            return {
                "jsonrpc": "2.0",
                "id": msg_id,
                "result": {
...
class MCPSseServer:
    ...
    def __init__(self, bridge: MCPBridge, host: str = "0.0.0.0", port: int = 0) -> None:
        self._bridge = bridge
        self._host = host
        self._port = port
    ...
    async def _handle_message(
        ...
        # Process asynchronously so tools can take time
